...
;;; Setup Procedures ;;;
;;;;;;;;;;;;;;;;;;;;;;;;
turtles-own [age religion classe politique langue tweet retweets]
...
to go
  ;; All the turtles tweet
  ask turtles [ send-tweet ]
  ;; Turtles process the tweets from this turn and retweets from last turn
  ask turtles [ process-tweets ]
  show-community detect-community
...
;; Create the network
to init_network
  create-turtles agent_number
  [
    set color 9.9
    set age ((random-float 87) + 13) ;; Age entre 13 et 100 ans (exclu)
    set religion (random 4) ;; 4 religions differentes (0, 1, 2, 3)
    set classe (random-float 100) ;; Classe economique de 0 a 100% (exclu) ex: 10% -> parmi les 10% les plus riches, 90% -> parmi les 90% les plus riches <-> 10% les plus pauvres
    set politique (random-float 4) ;; Choix politique de 0 a 4 (exclu): 0 extreme gauche, 1 gauche, 2 centre, 3 droite,  4 extreme droite (entre deux possibles)
    set langue []
    repeat ((random 3) + 1) ;; Uniformely, people can have between 1 and 3 languages they speak
    [
      set langue (lput (random 10) langue) ;; 10 different languages
    ]
...
;; At the beginning, all the nodes are processed in order to find the 5 that are most linked to the current node according to its beliefs and create links with those
to process-tweets
  ;; Create a list without the current turtle
  let others (turtles with [self != myself])
  let tmp_retweets []
  ask others
  [
    ;; randomly sees the tweet depending on wether or not agent follows other agetn
    ifelse((link-with myself) != nobody)
    [
      ;; 80% chance to see the tweet/retweet
      if(random 100 < 80)
      [
        ;; Check the ressemblance between the tweet and the agent that called ask others
        ;; If the tweet ressemble the agent at more than 70% then it retweets it
        if((ressemblance-tweet tweet myself) > 70)
        [
          ;; we add the provenance of the tweet to the retweet
          set (tmp_retweets) (lput (lput self tweet) tmp_retweets)
        ]
        ;; There is a link but the tweet is so bad that we unfollow
        if((ressemblance-tweet tweet myself) < 50)
        [
          ask link-with myself [ die ]
        ]
      ]
      if(random 100 < 80 and not empty? retweets)
      [
        ;; Check the ressemblance between the retweet and the agent that called ask others
        ;; If the tweet ressemble the agent at more than 70% then it retweets it
        let chosen_retweet (one-of retweets)
        if((ressemblance-tweet chosen_retweet myself) > 70)
        [
          set (tmp_retweets) (lput chosen_retweet tmp_retweets)
        ]
        if((ressemblance-tweet chosen_retweet myself) > 80 and (item 5 chosen_retweet) != myself)
        [
          ask myself[ create-link-with (item 5 chosen_retweet)]
          set (tmp_retweets) (lput chosen_retweet tmp_retweets)
        ]
        ;; There is a link but the tweet is so bad that we unfollow
        if((ressemblance-tweet chosen_retweet myself) < 50)
        [
          ask myself [ if((link-with (item 5 chosen_retweet)) != nobody) [ ask link-with (item 5 chosen_retweet) [die] ]]
        ]
      ]
    ]
    ;;Case where agent does not follow
    [
      ;; 20% chance to see the tweet
      if(random 100 < 20)
      [
        ;; Check the ressemblance between the tweet and the agent that called ask others
        ;; If the tweet ressemble the agent at more than 70% then it retweets it
        ...
        ;; There is no link but the tweet is so good that we follow
        if((ressemblance-tweet tweet myself) > 80)
        [
          create-link-with myself
        ]
      ]
      ;; 20% chance to see one of the retweets
      if(random 100 < 20 and not empty? retweets)
      [
        ;; Check the ressemblance between the retweet and the agent that called ask others
        ;; If the tweet ressemble the agent at more than 70% then it retweets it
        let chosen_retweet (one-of retweets)
  ...
  set retweets tmp_retweets
end

to send-tweet
  let tweet_age (seuil (random-normal age 15) 13 100)
  let tweet_religion religion
  let tweet_classe (seuil (random-normal classe 30) 0 100)
  let tweet_politique (seuil (random-normal politique 1) 0 4)
  let tweet_langue one-of langue
  set tweet (list tweet_age tweet_religion tweet_classe tweet_politique tweet_langue)
  if(retweets = 0) [set retweets []]
end

to-report seuil[nb minimum maximum]
  if(nb < minimum) [ report minimum]
  if(nb > maximum) [ report maximum]
  report nb
end

;; Calcul de la ressemblance entre 2 noeuds
to-report ressemblance [node]
  let ret 0
  ;; Age
  set ret (ret + (100 - 100 * abs(age - [age] of node) / 87))
  ;; Religion
  set ret (ret + (same-religion node) * 100)
  ;; Classe
  set ret (ret + (100 - 100 * abs(classe - [classe] of node) / 100))
  ;; Politique
  set ret (ret + (100 - 100 * abs(politique - [politique] of node) / 4))
  ;; Langue
  set ret (ret + (same-language node) * 100)
  report (ret / 5)
end

to-report ressemblance-tweet [l n]
  let ret 0
  ;; Age
  set ret (ret + (100 - 100 * abs((item 0 l) - [age] of n) / 87))
  ;; Religion
  set ret (ret + (same-rel-list (item 1 l) n) * 100)
  ;; Classe
  set ret (ret + (100 - 100 * abs((item 2 l) - [classe] of n) / 100))
  ;; Politique
  set ret (ret + (100 - 100 * abs((item 3 l) - [politique] of n) / 4))
  ;; Langue
  set ret (ret + (same-lan-list (item 4 l) n) * 100)
  report (ret / 5)
end

;; Same language spoke
to-report same-lan-list [l n]
  foreach [langue] of n
  [
    t ->
    if (l = t) [ report 1 ]
...
to-report same-rel-list[r n]
  ifelse (r != [religion] of n)
  [
    report 0
  ]
  [
...
to-report same-religion [node]
  ifelse (religion != [religion] of node)
  [
    report 0
  ]
  [
...
;; Same language spoke
to-report same-language [node]
  foreach langue
  [
    l ->
    if (member? l ([langue] of node)) [ report 1 ]
  ]
  report 0
end
...
to-report detect-community
  let nkernel 3
  let kernels (list)

  ;;retrieve list of turtles with most links
  repeat nkernel
  [
    let chosen_turtle nobody
    let chosen_nlink 0
    ask turtles [
      let nlink count my-links
      if (nlink >= chosen_nlink) and (not member? self kernels)
      [
       set chosen_turtle self
       set chosen_nlink nlink
      ]
    ]
    set kernels lput chosen_turtle kernels
  ]
  ...
  repeat nkernel
  [
    set partition (lput [] partition)
  ]
  ask turtles
  [
    let min_kernel nobody
    let min_val 10000
    let min_i 0
    let i 0
    show kernels
    foreach kernels ;;find nearest kernel for each turtle
    [
      k ->
      let dist ressemblance(k)
      if (dist < min_val)
      [
        set min_val dist
        set min_kernel k
    ...
    set (partition) (replace-item (min_i) partition (lput self (item (min_i) partition)))
  ]

  report partition
end

to show-community [partition]
